...
true "${sysconfdir:=/usr/local/etc}"

################################
# SAGAN-GROUPS
: "${FIFO:="/var/run/sagan.fifo"}"
: "${RULE_PATH:="$sysconfdir/sagan-rules"}"
...
CORE="sensor_name cluster_name default_host default_port default_proto dns_warnings source_lookup max_threads classification reference gen_msg_map protocol_map input_type json_map json_software"
...
rm -Rf ${sysconfdir:?}/sagan.yaml
touch ${sysconfdir:?}/sagan.yaml
{
  echo "%YAML 1.1"
  echo "---"
